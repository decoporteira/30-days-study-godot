...
func get_max_health():
	...
	var str_health = str(player.stats.health) + "/" + str(player.stats.get_max_health())
	life_bar_numbers.append_text(str_health)
	life_bar.max_value = player.stats.get_max_health()
	life_bar.value = player.stats.health
	
func update_life_bar(current, max_health):
	life_bar_numbers.clear()
	var str_health = str(current) + "/" + str(max_health)
	life_bar_numbers.append_text(str_health)
